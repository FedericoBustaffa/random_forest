...
from a recursive build. To improve prediction performance, some variant have
been tested like:
...
Both didn't bring any improvement to the prediction phase, probably due to the
fact that the DFS tree is compact enough to do not cause many cache misses. So
in order to keep the code cleaner and do not add the cost of a BFS conversion,
the choice end up being a simple DFS layout. So in the end the main optimization
to the preidiction phase was representing the tree as a compact array instead of
...
reused every time there is the need to analyze a new candidate threshold.
